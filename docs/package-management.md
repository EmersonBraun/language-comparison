---
sidebar_position: 27
description: "Package managers and dependency management compared across 12 programming languages"
keywords: [package management, npm, pip, cargo, go mod, NuGet, Maven, dependencies]
---

# Package Management

Every language has its own ecosystem for managing dependencies. Here's how different languages handle package management, dependency resolution, and project configuration.
...
- **Centralized vs decentralized registries** -- npm, PyPI, Crates.io, NuGet, and Maven Central are centralized; Go uses module proxies (defaulting to proxy.golang.org); Zig uses URL-based fetches with no central registry.
- **Lock files ensure reproducibility** -- Most ecosystems use lock files (package-lock.json, Cargo.lock, go.sum, Gemfile.lock, poetry.lock); C/C++ often rely on vendoring or system packages with less guarantees.
- **Tooling maturity varies** -- JavaScript has npm/yarn/pnpm; Python has pip plus Poetry/uv; Rust's Cargo is highly integrated; C and C++ use vcpkg, Conan, or CMake FetchContent with more manual setup.
- **Virtual environments isolate dependencies** -- Python's venv and Ruby's Bundler provide project-local isolation; Node's node_modules and Go's module cache serve similar goals.
- **C has no standard package manager** -- Developers typically use system package managers (apt, brew), vcpkg, or manual compilation; dependency management is largely manual.
