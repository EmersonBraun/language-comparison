---
sidebar_position: 18
description: "Manipulación de fechas y horas, formateo y manejo de zonas horarias en 12 lenguajes"
keywords: [fechas, hora, datetime, formateo, zona horaria, calendario]
---

# Fechas

La manipulación de fechas y horas es crucial en la mayoría de las aplicaciones. Así es como los diferentes lenguajes manejan las fechas, tanto con características incorporadas como con bibliotecas populares.

import LanguageComparison from '@site/src/components/LanguageComparison';

## Operaciones de Fecha Incorporadas
...
- **Madurez de la biblioteca estándar** — El `java.time` de Java, el `datetime` de Python, el `time` de Go y el `DateTime` de C# proporcionan APIs incorporadas ricas para parseo, formateo, aritmética y comparación. Por ejemplo, `time.Parse("2006-01-02", "2024-01-15")` de Go y `datetime.strptime("2024-01-15", "%Y-%m-%d")` de Python ambos parsean fechas sin dependencias extra. C, Zig y Rust ofrecen solo timestamps básicos en la biblioteca estándar; los desarrolladores de Rust típicamente usan el crate `chrono` para soporte completo de fechas. Si necesitas fechas directamente, elige Java, Python, Go o C#; para trabajo en sistemas, planea agregar una biblioteca de fechas.

- **Convenciones de formateo** — Cada lenguaje usa cadenas de formato diferentes: Go usa `2006-01-02 15:04:05` (fecha de referencia como mnemotécnico); Python usa `%Y-%m-%d %H:%M:%S`; C# usa `yyyy-MM-dd HH:mm:ss`; Java usa `DateTimeFormatter.ofPattern("yyyy-MM-dd")`. El enfoque de Go es memorable porque la fecha de referencia es literal: mes 01, día 02, hora 15, etc. Al cambiar de lenguaje, ten una hoja de referencia de formatos a mano—mezclarlos (por ejemplo, `%y` vs `%Y` para el año) causa bugs sutiles.

- **Manejo de zonas horarias** — El `ZonedDateTime` de Java, el `dateutil.tz` de Python, NodaTime de C# y chrono de Rust proporcionan soporte robusto de zonas horarias. El `Date` de JavaScript está centrado en UTC: `getHours()` retorna hora local, pero el almacenamiento es UTC, lo cual puede confundir a los desarrolladores. C y C++ requieren manejo manual con `localtime`/`gmtime` o bibliotecas externas. Para aplicaciones que necesitan mostrar o almacenar horas en múltiples zonas horarias, prefiere Java, Python o C# con NodaTime; evita C/C++ a menos que estés preparado para integrar una biblioteca de zonas horarias.

- **Bibliotecas populares** — date-fns (JavaScript), Carbon (PHP), chrono (Rust) y NodaTime (C#) extienden las funciones incorporadas con formateo legible por humanos, parseo natural y manejo más seguro de zonas horarias. Por ejemplo, el `$date->diffForHumans()` de Carbon retorna "2 hours ago." Zig y C tienen soporte mínimo del ecosistema—la mayoría de los proyectos usan `std.time` o timestamps Unix crudos. Si necesitas funciones avanzadas de fechas (por ejemplo, "próximo martes", "fin de mes"), elige un lenguaje con una biblioteca de fechas madura.
