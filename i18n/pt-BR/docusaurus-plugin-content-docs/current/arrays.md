---
sidebar_position: 11
description: "Arrays, listas, slices e operações de coleção comparados em 12 linguagens de programação"
keywords: [arrays, listas, slices, coleções, map, filter, reduce]
---
# Arrays

Arrays e listas são estruturas de dados fundamentais. Veja como diferentes linguagens lidam com criação, manipulação e iteração de arrays.
...
- **Fixo vs dinâmico** -- C, Zig e Go usam arrays de tamanho fixo (ex: `int arr[3]` em C, `var arr = [_]i32{1,2,3}` em Zig). JavaScript, Python, Ruby e PHP usam listas dinâmicas por padrão. Rust oferece ambos: `[T; N]` para tamanho fixo e `Vec<T>` para dinâmico. Escolha arrays fixos quando o tamanho é conhecido em tempo de compilação e você quer alocação na stack; use estruturas dinâmicas quando o tamanho varia em tempo de execução ou você precisa de redimensionamento frequente.

- **Slices** -- Slices de Go (`[]int`) são o tipo dinâmico principal e referenciam arrays subjacentes. Rust e Zig usam referências de slice (`&[T]`) para visualizações sem cópia. Python e C# usam sintaxe de slice (`arr[1:3]`, `arr[1..3]`) para subarrays. Slices evitam copiar dados — essencial para performance ao passar subintervalos. Prefira linguagens com suporte a slices ao construir parsers, processadores de stream ou APIs que frequentemente passam segmentos de arrays.

- **Map, filter, reduce** -- JavaScript, Python, Ruby e Swift fornecem métodos `map`, `filter` e `reduce` built-in. C# usa LINQ (`Select`, `Where`, `Aggregate`). Java usa Streams. Go e C exigem loops manuais. Escolha linguagens com primitivas funcionais de primeira classe ao fazer pipelines de transformação de dados; caso contrário, espere código imperativo mais verboso.

- **Operações de adicionar/remover** -- A maioria das linguagens usa `push`/`pop` para operações no final (ex: `arr.push(4)` em JavaScript, `vec.push(4)` em Rust). Go usa `append(arr, 4)` e fatiamento para remoção. O `ArrayList` de Zig requer alocação explícita e `try list.append(4)`. Considere a ergonomia da API: se você precisa de inserções frequentes em posições arbitrárias, o `list.insert()` de Python ou o `arr.unshift` de Ruby podem ser mais simples que a realocação manual de C.

- **Estilo funcional vs loops manuais** -- Linguagens como JavaScript e Python permitem encadear `arr.map(x => x*2).filter(x => x > 0)`. Go e C forçam loops explícitos. Java Streams fazem a ponte com `.map().filter().collect()`. Se sua equipe prefere código declarativo, favoreça JavaScript, Python ou Java; para controle máximo e abstração mínima, C ou Go funciona melhor.
