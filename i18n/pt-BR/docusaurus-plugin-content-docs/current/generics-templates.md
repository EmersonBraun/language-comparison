---
sidebar_position: 16
description: "Generics, templates e restrições de tipo comparados em 12 linguagens de programação"
keywords: [generics, templates, parâmetros de tipo, restrições, type-safe]
---

# Generics e Templates

Generics permitem que você escreva código reutilizável e com segurança de tipos. Veja como diferentes linguagens lidam com programação genérica.
...
- **Type erasure vs monomorfização** -- Java e TypeScript apagam generics em tempo de execução (uma representação compartilhada); C++, Rust, Swift e Go geram código especializado por tipo (monomorfização) para abstração de custo zero.
- **Sistemas de restrição variam em expressividade** -- As traits do Rust e os concepts do C++ oferecem bounds ricos; os conjuntos de tipo do Go (`~int | ~float64`) e as restrições de protocolo do Swift são mais simples; o `extends` e wildcards do Java são mais limitados.
- **Linguagens sem generics usam alternativas** -- C usa `void*` e macros; JavaScript/Ruby dependem de duck typing; PHP usa anotações de docblock para análise estática; Zig usa parâmetros de tipo comptime.
- **Generics comptime (Zig) são únicos** -- Zig passa tipos como valores em tempo de compilação, habilitando generics de custo zero sem uma sintaxe separada de parâmetros de tipo; tipos são de primeira classe em tempo de compilação.
- **Python e PHP têm generics tardios** -- Python 3.12+ e PHPStan/Psalm adicionam sintaxe de generics; a verificação de tipos é apenas estática — o comportamento em tempo de execução permanece inalterado — diferente de linguagens compiladas.
