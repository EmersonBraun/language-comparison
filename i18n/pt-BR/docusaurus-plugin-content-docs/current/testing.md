---
sidebar_position: 28
description: "Frameworks de testes unitários, asserções e mocking comparados em 12 linguagens de programação"
keywords: [testes, testes unitários, asserções, mocking, frameworks de teste, TDD]
---

# Testes

Testes unitários são fundamentais para a qualidade do software. Veja como diferentes linguagens lidam com testes usando seus frameworks embutidos ou mais populares.
...
- **Frameworks embutidos vs externos** -- Rust, Go e Zig vêm com test runners embutidos; JavaScript, Python, PHP e C++ dependem de ferramentas externas (Jest, pytest, PHPUnit, Google Test).
- **Estilos de asserção diferem** -- Go usa `if got != want { t.Errorf(...) }` manual; xUnit/Jest usam `Assert.Equal`/`expect().toBe`; RSpec e FluentAssertions favorecem estilo BDD `expect(x).to eq(y)`.
- **Abordagens de mocking variam** -- Mocking baseado em interface é comum em Go e Swift; Java/C# usam Mockito/Moq; Python usa `unittest.mock`; Rust usa crates como `mockall` com mocks baseados em traits.
- **Testes parametrizados são idiomáticos em alguns ecossistemas** -- Go favorece testes orientados a tabela; xUnit usa `[Theory]` e `[InlineData]`; JUnit 5 e Swift Testing têm `@ParameterizedTest` e `@Test(arguments:)`.
- **Testes de performance são embutidos em Go** -- `go test -bench=.` executa benchmarks; outras linguagens tipicamente usam ferramentas separadas (pytest-benchmark, criterion para Rust, etc.).
