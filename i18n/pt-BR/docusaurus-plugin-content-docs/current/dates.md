---
sidebar_position: 18
description: "Manipulação de data e hora, formatação e tratamento de fuso horário em 12 linguagens"
keywords: [datas, hora, datetime, formatação, fuso horário, calendário]
---

# Datas

A manipulação de data e hora é crucial na maioria das aplicações. Veja como diferentes linguagens lidam com datas, tanto com recursos nativos quanto com bibliotecas populares.

import LanguageComparison from '@site/src/components/LanguageComparison';

## Operações Nativas com Datas
...
- **Maturidade da biblioteca padrão** — O `java.time` do Java, o `datetime` do Python, o `time` do Go e o `DateTime` do C# fornecem APIs nativas ricas para análise, formatação, aritmética e comparação. Por exemplo, o `time.Parse("2006-01-02", "2024-01-15")` do Go e o `datetime.strptime("2024-01-15", "%Y-%m-%d")` do Python analisam datas sem dependências extras. C, Zig e Rust oferecem apenas timestamps básicos na biblioteca padrão; desenvolvedores Rust geralmente usam o crate `chrono` para suporte completo a datas. Se você precisa de datas prontas para uso, escolha Java, Python, Go ou C#; para trabalho em sistemas, planeje adicionar uma biblioteca de datas.

- **Convenções de formatação** — Cada linguagem usa strings de formato diferentes: Go usa `2006-01-02 15:04:05` (data de referência como mnemônico); Python usa `%Y-%m-%d %H:%M:%S`; C# usa `yyyy-MM-dd HH:mm:ss`; Java usa `DateTimeFormatter.ofPattern("yyyy-MM-dd")`. A abordagem do Go é memorável porque a data de referência é literal: mês 01, dia 02, hora 15, etc. Ao trocar de linguagem, mantenha uma tabela de referência de formatos à mão — misturar esses formatos (ex.: `%y` vs `%Y` para ano) causa bugs sutis.

- **Tratamento de fuso horário** — O `ZonedDateTime` do Java, o `dateutil.tz` do Python, o NodaTime do C# e o chrono do Rust fornecem suporte robusto a fusos horários. O `Date` do JavaScript é centrado em UTC: `getHours()` retorna hora local, mas o armazenamento é UTC, o que pode confundir desenvolvedores. C e C++ exigem tratamento manual com `localtime`/`gmtime` ou bibliotecas externas. Para aplicações que precisam exibir ou armazenar horários em múltiplos fusos, prefira Java, Python ou C# com NodaTime; evite C/C++ a menos que esteja preparado para integrar uma biblioteca de fusos horários.

- **Bibliotecas populares** — date-fns (JavaScript), Carbon (PHP), chrono (Rust) e NodaTime (C#) estendem os recursos nativos com formatação legível, análise natural e tratamento mais seguro de fusos horários. Por exemplo, o `$date->diffForHumans()` do Carbon retorna "2 hours ago." Zig e C têm suporte mínimo de ecossistema — a maioria dos projetos usa `std.time` ou timestamps Unix brutos. Se você precisa de recursos avançados de data (ex.: "próxima terça-feira", "fim do mês"), escolha uma linguagem com uma biblioteca de datas madura.
