---
sidebar_position: 13
description: "Objetos, structs e tipos de dados estruturados comparados em 12 linguagens de programação"
keywords: [objetos, structs, estruturas, propriedades, métodos]
---
# Objetos & Structs

Diferentes linguagens têm diferentes formas de representar dados estruturados. Veja como objetos, structs e construções similares funcionam em diferentes linguagens.

import LanguageComparison from '@site/src/components/LanguageComparison';

## Criação de Objetos/Structs
...
- **Struct vs classe vs dicionário** -- Swift, Rust, Go, Zig e C usam structs como contêineres de dados primários para campos nomeados. JavaScript, Python, PHP e Ruby frequentemente usam objetos simples ou dicionários (ex: `{ name: "John", age: 30 }`) para dados ad-hoc. Java e C# favorecem classes para dados estruturados. Use structs quando precisar de schemas fixos e verificação em tempo de compilação; use dicionários quando o schema é dinâmico ou você está construindo configuração ou dados similares a JSON.

- **Semântica de valor vs referência** -- Structs de Swift e Rust são tipos de valor: `let b = a` copia os dados. Objetos de Java, C# e JavaScript são tipos de referência: `b = a` compartilha a mesma instância. Structs de Go são valores, mas são comumente passados como ponteiros (`*Person`) para mutação. Semântica de valor evita mutação compartilhada acidental; semântica de referência reduz cópias. Escolha tipos de valor (Swift, Rust) para comportamento previsível e segurança de threads; tipos de referência quando precisar de estado mutável compartilhado.

- **Anexação de métodos** -- Rust usa `impl Person { fn greet(&self) {...} }`; Go usa sintaxe de receiver `func (p Person) Greet() string`; Zig incorpora funções dentro de structs. JavaScript e Python anexam métodos a classes. C usa funções externas como `greet(&person)`. O padrão afeta como você organiza o código: blocos `impl` co-localizam lógica com dados em Rust; Go separa métodos de definições de tipo. Prefira linguagens com sintaxe de métodos ao construir modelos de domínio.

- **Alternativas leves** -- `@dataclass` de Python, `record` de C# e `record` de Java fornecem definições compactas para contêineres de dados sem boilerplate. JavaScript e Ruby usam object literals ou hashes. Para DTOs, objetos de configuração ou payloads de API, prefira dataclasses ou records; para lógica de negócio rica, use classes ou structs completas com métodos.

- **Serialização e reflexão** -- Rust e Go usam tags de struct (`#[derive(Serialize)]`, `` `json:"name"` ``) para JSON. `dataclass` de Python e `asdict()` tornam a serialização fácil. C e Zig exigem mapeamento manual. Se você frequentemente serializa para JSON ou outros formatos, favoreça linguagens com suporte built-in (Go, Rust com serde, Python, C#); para runtime mínimo, C ou Zig podem ser aceitáveis.
